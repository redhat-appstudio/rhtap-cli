...
update_charts() {
    ...
    find installer/charts/ -name Chart.yaml | while read CHART; do
...
update_template() {
    ...
    get_version "redHatDeveloperHub"
    CATALOG_URL="https://github.com/redhat-appstudio/tssc-sample-templates/blob/$VERSION_XYZ/all.yaml" \
...
update_ci() {
    for PLR in ".tekton/rhtap-cli-pull-request.yaml" ".tekton/rhtap-cli-push.yaml"; do
        ...
        sed -i "s|  *appstudio\.openshift\.io/application: rhtap-cli|\0-$RELEASE_BRANCH|" "$PLR"
        sed -i "s|  *appstudio\.openshift\.io/component: rhtap-cli|\0-$RELEASE_BRANCH|" "$PLR"
    done
}
