...
acs_quay_connection() {
  ...
  if [[ "${quay_install_enabled}" == "true" ]]; then
    echo "[INFO] Configure internal Quay integration with ACS"

    # Get ACS endpoint url and token from secret 'rhtap-acs-integration' for both Internal and External ACS
    acs_central_url=https://$(kubectl -n rhtap get secret rhtap-acs-integration -o go-template='{{index .data "endpoint" | base64decode}}')
    acs_api_token=$(kubectl -n rhtap get secret rhtap-acs-integration -o go-template='{{index .data "token" | base64decode}}')
