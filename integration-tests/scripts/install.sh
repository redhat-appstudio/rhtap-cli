...
update_dh_catalog_url() {
  ...
  if [[ -n "${DEVELOPER_HUB__CATALOG__URL}" ]]; then
    ...
    yq -i ".cli.features.redHatDeveloperHub.properties.catalogURL = strenv(DEVELOPER_HUB__CATALOG__URL)" "${config_file}"
  fi
}
...
disable_quay() {
  ...
  if [[ "${registry_config}" != "quay" ]]; then
    ...
    yq e '.cli.features.redHatQuay.enabled = false' -i "${config_file}"
  fi
}
...
disable_acs() {
  if [[ "${acs_config}" == "hosted" ]]; then
    ...
    yq e '.cli.features.redHatAdvancedClusterSecurity.enabled = false' -i "${config_file}"
  fi
}
...
disable_tpa() {
  if [[ "${tpa_config}" == "hosted" ]]; then
    ...
    yq e '.cli.features.trustedProfileAnalyzer.enabled = false' -i "${config_file}"
  fi
}
