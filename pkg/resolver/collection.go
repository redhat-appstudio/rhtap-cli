...
func (c *Collection) Walk(fn CollectionWalkFn) error {
	for name, hc := range c.charts {
		if err := fn(name, hc); err != nil {
			return err
		}
	}
	return nil
...
func (c *Collection) DependsOn(hc *chart.Chart) []string {
	if dependsOn, exists := hc.Metadata.Annotations[DependsOnAnnotation]; exists {
		return strings.Split(dependsOn, ", ")
	}
	return nil
}
