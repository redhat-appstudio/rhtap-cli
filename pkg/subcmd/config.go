...
type Config struct {
	...
	update bool // update an existing configuration
...
configuration file path to "--apply" and "--update".

The "--apply" flag reflects the creation of a new configuration while, "--update"
...
func (c *Config) validateFlags() error {
	...
	if !c.create && !c.update && !c.get && !c.delete {
		return fmt.Errorf("either apply, update, get or delete must be set")
	}
	return nil
}

// Complete inspect the context to determin the path of the configuration file, or
// uses the embedded payload, makes sure the args are adequate.
func (c *Config) Complete(args []string) error {
	// It should return error more than a single argument is informed.
	if len(args) > 1 {
		return fmt.Errorf("unexpected arguments: %v", args)
	}
	// It should inform a configuration file only for apply and update flags.
	if (c.get || c.delete) && (!c.create && !c.update) && len(args) > 0 {
		return fmt.Errorf(
			"configuration file is only permitted during --apply or --update")
	}
...
// runCreate runs create action, or apply, makes sure a new configuration is
// applied in the cluster, or shows error to inform the user about "--update"
// flag.
func (c *Config) runCreate() error {
	...
	if err = c.manager.Create(c.cmd.Context(), cfg); err != nil {
		if apierrors.IsAlreadyExists(err) {
			return fmt.Errorf(
				"the configuration already exists, use --update to amend it")
		}
	}
	return err
}

// runUpdate runs the update action, loads the configuration from CFS and updates
// the cluster.
func (c *Config) runUpdate() error {
	c.log().Debug("Loading configuration from file")
	cfg, err := config.NewConfigFromFile(c.cfs, c.configPath)
	if err != nil {
		return err
...
// NewConfig instantiates the a new "config" subcommand.
func NewConfig(
	logger *slog.Logger,
	cfs *chartfs.ChartFS,
	kube *k8s.Kube,
